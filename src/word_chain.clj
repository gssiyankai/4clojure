...
(defn remove-common-leading-chars [[word1 word2]]
  (if (or (empty? word1)
          (empty? word2)
...
(defn remove-common-trailing-chars [[word1 word2]]
  (map
    #(apply str (reverse %))
...
(defn strip [[word1 word2]]
  (remove-common-trailing-chars
    (remove-common-leading-chars [word1 word2]))
)

(defn can-chain-with-modification [size word1 word2]
  (= size (map count (strip [word1 word2])))
)

(defn can-chain-with-substitution [word1 word2]
  (can-chain-with-modification [1 1] word1 word2)
)

(defn can-chain-with-insertion [word1 word2]
  (can-chain-with-modification [0 1] word1 word2)
)

(defn can-chain-with-deletion [word1 word2]
  (can-chain-with-insertion word2 word1)
)

(defn can-chain
  ([words] (can-chain (set (rest words)) (first words) nil))
  ([words first_chain_word last_chain_word]
    (let [chain-words (fn [word] (filter #(or (can-chain-with-substitution % word)
